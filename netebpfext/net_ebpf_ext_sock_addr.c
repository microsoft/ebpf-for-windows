    ...
    _In_ const FWPS_INCOMING_VALUES* incoming_fixed_values, _Out_ bpf_sock_addr_t* sock_addr_ctx)
{
    ...
    if ((hook_id == EBPF_HOOK_SOCK_ADDR_CONNECT_V4) || (hook_id == EBPF_HOOK_SOCK_ADDR_RECV_ACCEPT_V4)) {
        ...
        sock_addr_ctx->msg_src_ip4 = _byteswap_ulong(incoming_values[source_ip_address_field].value.uint32);
        sock_addr_ctx->user_ip4 = _byteswap_ulong(incoming_values[destination_ip_address_field].value.uint32);
    } else {
    ...
    sock_addr_ctx->msg_src_port = _byteswap_ushort(incoming_values[source_port_field].value.uint16);
    sock_addr_ctx->user_port = _byteswap_ushort(incoming_values[destination_port_field].value.uint16);
