...
- You need to submit the **merged `.hlkx` package** for manual signing, from all the other HLK Controllers, into one of your choice.
- Submit a new request in [ESRP Portal](https://portal.esrp.microsoft.com/), using the `OPC Sign` operation created initially in by following ["Signing drivers, and driver packages for submission to HDC - OSGWiki"](https://www.osgwiki.com/wiki/Signing_drivers,_and_driver_packages_for_submission_to_HDC).
- You need 2 reviewers to approve your manual submission.
...
#### **TBC, as my account won't allow the below operations!**
