 ...
 *  ref-count > 0 (if their pointers in their respective id
 *  table entries are non-null).
 *
...
typedef struct _ebpf_id_entry
{
    ...
    //   pointed-to object (via it's id) and will need to handle the failure gracefully.
    //
    ...
    ebpf_core_object_t* object)
{
    ...
    // Under lock, so un-protected access if ok.
...
ebpf_object_acquire_id_reference(ebpf_id_t id, ebpf_object_type_t object_type)
{
    ...
    if (entry->object == NULL) {
        ...
        // The object at this entry has been deleted and all that remains are (now stale) references to this entry
        // held by other objects.  This is a non-reversible situation and there's no point in giving out references
        // for such entries, so deny with suitable error code.
