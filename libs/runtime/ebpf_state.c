...
ebpf_state_initiate()
{
    ...
    _ebpf_state_cpu_table =
        cxplat_allocate_cache_aligned(sizeof(ebpf_state_entry_t) * _ebpf_state_cpu_table_size, EBPF_POOL_TAG_STATE);
...
ebpf_state_terminate()
{
    ...
    cxplat_free(_ebpf_state_cpu_table, EBPF_POOL_TAG_STATE);
