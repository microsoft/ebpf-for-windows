...
ebpf_epoch_initiate()
{
    ...
    _ebpf_epoch_cpu_table =
        cxplat_allocate_cache_aligned(sizeof(ebpf_epoch_cpu_entry_t) * _ebpf_epoch_cpu_count, EBPF_POOL_TAG_EPOCH);
...
ebpf_epoch_terminate()
{
    ...
    cxplat_free(_ebpf_epoch_cpu_table, EBPF_POOL_TAG_EPOCH);
