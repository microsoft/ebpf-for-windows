    ...
    _Inout_ ebpf_ring_buffer_t* ring_buffer, _In_ ebpf_handle_t wait_handle, uint64_t flags)
{
    ...
    NTSTATUS status = ObReferenceObjectByHandle(
        (HANDLE)wait_handle,
        EVENT_MODIFY_STATE,
        NULL, // TODO: *ExEventObjectType is not implemented in usersim.
        UserMode,
        (PVOID*)&wait_event,
        NULL);
...
ebpf_ring_buffer_get_wait_handle(_In_ ebpf_ring_buffer_t* ring_buffer)
{
    ...
    // TODO: Usersim doesn't implement ObOpenObjectByPointer.
...
ebpf_ring_buffer_map_user(
    _In_ const ebpf_ring_buffer_t* ring, _Outptr_ uint8_t** buffer, uint32_t offset, uint32_t size, uint32_t page_protection)
{
    *buffer = ebpf_ring_map_user(ring->ring_descriptor, offset, size, page_protection);
    if (!*buffer) {
        return EBPF_INVALID_ARGUMENT;
    } else {
        return EBPF_SUCCESS;
    ...
    _When_(return != NULL, _Out_) size_t* next_offset)
{
    ...
    if (record) {
        return (ebpf_ring_buffer_record_t*)(buffer + ((uint8_t*)record - (uint8_t*)ring_buffer->consumer_page));
    } else {
        return NULL;
    }
}
    ...
    _Inout_ ebpf_ring_buffer_t* ring, _Outptr_result_bytebuffer_(length) uint8_t** data, size_t length)
{
    ...
    for (;;) {
        ...
        if (old_reserve_offset == reserve_offset) {
            ...
            record->header.page_offset = (uint32_t)(reserve_offset % _ring_get_length(ring)) / PAGE_SIZE;
    ...
    _Inout_ ebpf_ring_buffer_t* ring, _Outptr_result_bytebuffer_(length) uint8_t** data, size_t length)
{
    ...
    ebpf_ring_buffer_record_t* record = _ring_record_at_offset(ring, reserve_offset);
    record->header.page_offset = (uint32_t)(reserve_offset % _ring_get_length(ring)) / PAGE_SIZE; // unused for now.
...
{
    return ring_buffer->consumer_page;
}
