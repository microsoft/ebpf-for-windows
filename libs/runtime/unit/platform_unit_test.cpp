...
// TODO (before merge): refactor missing tests into execution context unit tests
//  size_t
...
//      _Analysis_assume_(perf_event_array != nullptr); // This is missed by Analyze build, but asserted by REQUIRE
//      above. REQUIRE(ebpf_perf_event_array_map_buffer(perf_event_array, cpu_id, &buffer) == EBPF_SUCCESS);
...
//      REQUIRE(consumer == 0);
//
//      REQUIRE(ebpf_perf_event_array_output(ctx, perf_event_array, flags, data.data(), data.size(), NULL) ==
//      EBPF_SUCCESS); ebpf_perf_event_array_query(perf_event_array, cpu_id, &consumer, &producer);
//
//      // Ring is not empty.
...
//
//      data.resize(1023);
//      while (ebpf_perf_event_array_output(ctx, perf_event_array, flags, data.data(), data.size(), NULL) ==
//      EBPF_SUCCESS) {
//          if (++write_count > 1000) {
...
//          // Output an event.
//          REQUIRE(
//              ebpf_perf_event_array_output(ctx, perf_event_array, flags, data.data(), data.size(), NULL) ==
//              EBPF_SUCCESS);
//
...
//  #define STRINGIZE2(x) #x
//  #define STRINGIZE(x) STRINGIZE2(x)
//  #def i ne PERF_TEST_CASE(cpu, use_c urrent_cpu, ctx_data_len, data_len, capture_len, expected) \
//     {cpu, \
//      use_current_cpu, \
//      ctx_data_len, \
//      data_len, \
//      capture_len, \
//      expected, \
//      "Line " STRINGIZE(__LINE__) ": {" #cpu ", " #use_current_cpu ", " #ctx_data_len ", " #data_len ", "
//       #capture_len \
//                                  ", " #expected "}"}
...
//              test_flags |= ((uint64_t)test->capture_length << EBPF_MAP_FLAG_CTXLEN_SHIFT) &
//              EBPF_MAP_FLAG_CTXLEN_MASK;
//          }
//          _test_perf_event_output(
//              perf_event_array,
//              buffers[test->cpu_id],
