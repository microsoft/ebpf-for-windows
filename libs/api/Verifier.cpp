...
collect_stats(const prevail::Program& prog)
{
    ...
    for (const char* stat_name : {"instructions", "joins", "other", "jumps", "assign", "arith", "load", "store",
                          "load_store", "packet_access", "call_1", "call_mem", "call_nomem", "reallocate",
                          "map_in_map", "arith64", "arith32"}) {
        res[stat_name] = 0;
    }
