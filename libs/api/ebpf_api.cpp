...
typedef struct _ebpf_pe_context
{
    ...
    std::map<std::string, std::string> section_hash_types;
    ...
    _In_ const bounded_buffer* buffer) noexcept
{
    ...
    if (section_name == "programs") {
        ...
        for (int i = 0; i < program_count; i++) {
            ...
            if (program->program_info_hash_type) {
                ...
                pe_context->section_hash_types[pe_section_name] = program_info_hash_type;
            }
        }
    }
    ...
    const bounded_buffer* buffer) noexcept
{
    ...
    std::string program_hash_type = pe_context->section_hash_types[pe_section_name];
