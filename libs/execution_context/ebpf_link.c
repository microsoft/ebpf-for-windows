...
_ebpf_link_instance_invoke_batch_begin(size_t state_size, _Out_writes_(state_size) void* state)
{
    ...
    ebpf_event_write(ebpf_link_invoke_batch_begin_entry, NULL);
...
Done:
    ...
    ebpf_event_write(ebpf_link_invoke_batch_begin_exit, NULL, return_value);
...
{
    ebpf_execution_context_state_t* execution_context_state = (ebpf_execution_context_state_t*)state;
    ebpf_event_write(ebpf_link_invoke_batch_end_entry, NULL);
    ebpf_assert_success(ebpf_state_store(ebpf_program_get_state_index(), 0, execution_context_state));
    ebpf_epoch_exit((ebpf_epoch_state_t*)(execution_context_state->epoch_state));
    ebpf_event_write(ebpf_link_invoke_batch_end_exit, NULL);
    return EBPF_SUCCESS;
}
    ...
    _In_ const void* state)
{
    ...
    ebpf_event_write(ebpf_link_instance_invoke_batch_entry, NULL, link);

    return_value =
        ebpf_program_invoke(link->program, false, program_context, result, (ebpf_execution_context_state_t*)state);

    ebpf_event_write(ebpf_link_instance_invoke_batch_exit, NULL, return_value);
...
_ebpf_link_instance_invoke_batch_begin_with_context_header(size_t state_size, _Out_writes_(state_size) void* state)
{
    ...
    ebpf_event_write(ebpf_link_invoke_batch_begin_with_context_header_entry, NULL);
...
Done:
    ...
    ebpf_event_write(ebpf_link_invoke_batch_begin_with_context_header_exit, NULL, return_value);
...
{
    ebpf_execution_context_state_t* execution_context_state = (ebpf_execution_context_state_t*)state;
    ebpf_event_write(ebpf_link_invoke_batch_end_with_context_header_entry, NULL);
    ebpf_epoch_exit((ebpf_epoch_state_t*)(execution_context_state->epoch_state));
    ebpf_event_write(ebpf_link_invoke_batch_end_with_context_header_exit, NULL);
    return EBPF_SUCCESS;
}
    ...
    _In_ const void* state)
{
    ...
    ebpf_event_write(ebpf_link_instance_invoke_batch_with_context_header_entry, NULL, link);

    return_value =
        ebpf_program_invoke(link->program, true, program_context, result, (ebpf_execution_context_state_t*)state);

    ebpf_event_write(ebpf_link_instance_invoke_batch_with_context_header_exit, NULL, return_value);
