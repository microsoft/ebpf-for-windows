...
#define EBPF_MAP_FIND_FLAG_DELETE 0x02 /* Perform a find and delete. */
     ...
     * @param[in] wait_handle Handle to notify when the ring buffer is full.
    ...
    /**
     * @brief Get the notify handle for a ring buffer map.
     *
     * @param[in] map Ring buffer map to set the notify handle for.
     * @param[out] wait_handle Handle to notify when the ring buffer is full.
     * @retval EBPF_SUCCESS Successfully got the notify handle.
     * @retval EBPF_INVALID_ARGUMENT Unable to get the notify handle.
     */
    _Must_inspect_result_ ebpf_result_t
