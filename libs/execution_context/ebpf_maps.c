    ...
    ebpf_map_option_t option)
{
    ...
    if (!map || !key || (option == EBPF_NOEXIST)) {
        EBPF_LOG_MESSAGE(EBPF_TRACELOG_LEVEL_ERROR, EBPF_TRACELOG_KEYWORD_MAP, "Invalid argument(s) rejected");
        return EBPF_INVALID_ARGUMENT;
    }
    ...
    if (value_object) {
        ...
        // Acquire a reference to the id table entry for the new incoming id.
        result = ebpf_object_acquire_id_reference(value_object->id, value_type);
        ebpf_assert(result == EBPF_SUCCESS);
        if (result != EBPF_SUCCESS) {

            // This can _never_ happen as we already have a valid pointer to the object.  This is indicative of a
    ...
    _Inout_ ebpf_core_map_t* map, _In_ const uint8_t* key, ebpf_object_type_t value_type)
{
    if (value_type == EBPF_OBJECT_UNKNOWN) {
        EBPF_LOG_MESSAGE(EBPF_TRACELOG_LEVEL_ERROR, EBPF_TRACELOG_KEYWORD_MAP, "Unknown value type rejected");
        return EBPF_INVALID_ARGUMENT;
    }
...
_get_object_from_array_map_entry(_Inout_ ebpf_core_map_t* map, _In_ const uint8_t* key)
{
    ...
    if (_find_array_map_entry(map, (uint8_t*)&index, false, &value) == EBPF_SUCCESS) {
        ...
        if (id != 0) {
            ...
            // Note that this call might fail and that's fine.  The id might be valid, but the object might since have
            // been deleted.
...
{
    if (!map || !key) {
        EBPF_LOG_MESSAGE(EBPF_TRACELOG_LEVEL_ERROR, EBPF_TRACELOG_KEYWORD_MAP, "Invalid argument(s)");
        return EBPF_INVALID_ARGUMENT;
    }
