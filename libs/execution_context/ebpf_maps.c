...
_associate_inner_map(_Inout_ ebpf_core_object_map_t* object_map, ebpf_handle_t inner_map_handle)
{
    ...
    result = ebpf_object_reference_by_handle_and_type(inner_map_handle, EBPF_OBJECT_MAP, &inner_map_template_object);
    ...
    ebpf_map_option_t option)
{
    ...
    if (value_handle != (uintptr_t)ebpf_handle_invalid) {
        result = ebpf_object_reference_by_handle_and_type(value_handle, value_type, &value_object);
        if (result != EBPF_SUCCESS) {
            return result;
        }
    }
