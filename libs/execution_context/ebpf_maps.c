...
_reap_oldest_map_entry(_In_ ebpf_core_map_t* map)
{
    ...
    if (result == EBPF_NO_MORE_KEYS && oldest_key != NULL) {
        ...
        if (ebpf_hash_table_delete((ebpf_hash_table_t*)lru_map->core_map.data, oldest_key) == EBPF_SUCCESS) {
    ...
    _In_ void* async_context)
{
    ...
    result = ebpf_async_set_cancel_callback(async_context, context, _ebpf_ring_buffer_map_cancel_async_query);
    if (result != EBPF_SUCCESS) {
        ebpf_free(context);
        goto Exit;
    }
