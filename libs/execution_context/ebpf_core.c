...
static ebpf_result_t
_ebpf_core_protocol_link_disconnect(_In_ const ebpf_operation_link_disconnect_request_t* request)
{
    EBPF_LOG_ENTRY();
    ebpf_result_t result;
    ebpf_link_t* link = NULL;
    ...
    result = ebpf_link_disconnect(link);
...
static ebpf_protocol_handler_t _ebpf_protocol_handlers[] = {
    ...
    DECLARE_PROTOCOL_HANDLER_FIXED_REQUEST_NO_REPLY(link_disconnect, PROTOCOL_ALL_MODES),
};
