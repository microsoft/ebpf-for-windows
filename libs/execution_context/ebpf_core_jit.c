...
_Must_inspect_result_ ebpf_result_t
ebpf_core_protocol_set_btf_resolved_functions(_In_ const ebpf_operation_set_btf_resolved_functions_request_t* request)
{
    EBPF_LOG_ENTRY();

    ebpf_result_t return_value;
    size_t data_length = 0;
    size_t function_count = request->count;
    btf_resolved_function_entry_t* btf_resolved_functions = NULL;
    const uint8_t* current;
    const uint8_t* end;

    return_value = ebpf_safe_size_t_subtract(
    ...
    if (function_count > 0) {
        ...
        entries_length = function_count * sizeof(*btf_resolved_functions);

        btf_resolved_functions =
            (btf_resolved_function_entry_t*)ebpf_allocate_with_tag(entries_length, EBPF_POOL_TAG_CORE);
        if (btf_resolved_functions == NULL) {
            return_value = EBPF_NO_MEMORY;
            goto Done;
        }
        memset(btf_resolved_functions, 0, entries_length);
        __analysis_assume(entries_length >= function_count * sizeof(*btf_resolved_functions));
    }
    ...
    for (size_t index = 0; index < function_count; index++) {
        ...
        if (((size_t)(end - current) < required_entry_length) || (serialized_entry->name[name_length] != '\0')) {
            return_value = EBPF_INVALID_ARGUMENT;
            goto Done;
        }

        btf_resolved_functions[index].name = serialized_entry->name;
        btf_resolved_functions[index].module_guid = serialized_entry->module_guid;
        btf_resolved_functions[index].return_type = serialized_entry->return_type;
        memcpy(
            btf_resolved_functions[index].arguments,
            serialized_entry->arguments,
            sizeof(btf_resolved_functions[index].arguments));
        btf_resolved_functions[index].flags = serialized_entry->flags;
        current += required_entry_length;
    }
    ...
    __analysis_assume(function_count == 0 || btf_resolved_functions != NULL);
    return_value =
        ebpf_core_set_btf_resolved_function_entries(request->program_handle, function_count, btf_resolved_functions);

Done:
    ebpf_free(btf_resolved_functions);
    EBPF_RETURN_RESULT(return_value);
}
