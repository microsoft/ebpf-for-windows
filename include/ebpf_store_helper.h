    ...
    _In_reads_(program_info_count) ebpf_program_info_t* program_info, uint32_t program_info_count)
{
    ...
    for (uint32_t i = 0; i < program_info_count; i++) {
        ...
        // Save helper count.
        status = write_registry_value_dword(
            program_key, EBPF_PROGRAM_DATA_HELPER_COUNT, program_info[i].count_of_program_specific_helpers);
        if (!IS_SUCCESS(status)) {
            close_registry_key(program_key);
            goto Exit;
        }

        if (program_info[i].count_of_program_specific_helpers != 0) {
            ...
            for (uint32_t count = 0; count < program_info[i].count_of_program_specific_helpers; count++) {
                status = _update_helper_prototype(
                    helper_info_key, &(program_info[i].program_specific_helper_prototype[count]));
                if (!IS_SUCCESS(status)) {
                    close_registry_key(program_key);
