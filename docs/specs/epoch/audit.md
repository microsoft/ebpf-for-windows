...
- [KNOWN] The user clarified that `validation.md` should be interpreted as the target validation baseline, so planned-but-unimplemented tests count as valid validation intent rather than document drift. Evidence: user clarification on 2026-06-30.
...
- [KNOWN] The initial apparent drift came from treating the validation artifact as a current-state test inventory instead of a target validation plan. Once the artifact semantics were clarified, the requirements-to-validation traceability gaps disappeared. Evidence: user clarification on 2026-06-30; `docs\specs\epoch\validation.md:3-5,16-27`.
...
3. [KNOWN] Preserve the user-confirmed contract choices—supported diagnostic API, nested entry/exit, and callback-outside-epoch semantics—in the final requirements baseline.
...
## 9. Revision History
- [KNOWN] Version 0.2 — 2026-06-30 — Reissued after clarifying that `validation.md` is a target validation plan rather than a current-state coverage report.
