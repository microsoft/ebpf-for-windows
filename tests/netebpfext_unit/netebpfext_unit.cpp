...
// Invoke SOCK_ADDR_RECV_ACCEPT concurrently with same classify parameters
TEST_CASE("sock_addr_recv_accept_invoke_concurrent1", "[netebpfext_concurrent][recv_accept]")
{
    ebpf_extension_data_t npi_specific_characteristics = {
        .header = EBPF_ATTACH_CLIENT_DATA_HEADER_VERSION,
    };
...
// Test comprehensive recv_accept scenarios including edge cases and authorization patterns.
TEST_CASE("sock_addr_recv_accept_comprehensive_test", "[netebpfext][recv_accept]")
{
    // This test validates comprehensive recv_accept functionality including
    // edge cases, authorization patterns, and proper WFP integration.
    ebpf_extension_data_t npi_specific_characteristics = {
        .header = EBPF_ATTACH_CLIENT_DATA_HEADER_VERSION,
    };
    test_sock_addr_client_context_header_t client_context_header = {0};
    ...
    // Test hard permit with validation enabled
    client_context->sock_addr_action = SOCK_ADDR_TEST_ACTION_PERMIT_HARD;
    client_context->validate_sock_addr_entries = true;
    ...
    // Test standard IPv4 recv_accept hard permit
    FWP_ACTION_TYPE result = helper.test_cgroup_inet4_recv_accept(&parameters);
    REQUIRE(result == FWP_ACTION_PERMIT);

    // Test IPv6 recv_accept hard permit
    result = helper.test_cgroup_inet6_recv_accept(&parameters);
    REQUIRE(result == FWP_ACTION_PERMIT);

    // Test soft permit scenarios
    client_context->sock_addr_action = SOCK_ADDR_TEST_ACTION_PERMIT_SOFT;
    result = helper.test_cgroup_inet4_recv_accept(&parameters);
    REQUIRE(result == FWP_ACTION_PERMIT);
    ...
    // Test redirect scenarios with recv_accept
    client_context->sock_addr_action = SOCK_ADDR_TEST_ACTION_REDIRECT;
    result = helper.test_cgroup_inet4_recv_accept(&parameters);
    REQUIRE(result == FWP_ACTION_PERMIT);
    ...
    // Test round-robin behavior with various port patterns
    client_context->sock_addr_action = SOCK_ADDR_TEST_ACTION_ROUND_ROBIN;
    client_context->validate_sock_addr_entries = false;

    // Test different port ranges to verify round-robin distribution
    std::vector<uint16_t> test_ports = {1000, 1001, 1002, 1003, 2000, 2001, 3000, 3001};

    for (uint16_t port : test_ports) {
        parameters.destination_port = htons(port);

        FWP_ACTION_TYPE expected_action = _get_fwp_sock_addr_action(port);

        FWP_ACTION_TYPE ipv4_result = helper.test_cgroup_inet4_recv_accept(&parameters);
        REQUIRE(ipv4_result == expected_action);

        FWP_ACTION_TYPE ipv6_result = helper.test_cgroup_inet6_recv_accept(&parameters);
        REQUIRE(ipv6_result == expected_action);
    }

    // Test failure scenario
    client_context->sock_addr_action = SOCK_ADDR_TEST_ACTION_FAILURE;

    // Note: Failure scenarios may not return consistent FWP_ACTION_TYPE results
    // as they represent internal errors, but the test should not crash
    helper.test_cgroup_inet4_recv_accept(&parameters);
    helper.test_cgroup_inet6_recv_accept(&parameters);
}

// Test advanced recv_accept validation scenarios and context handling.
TEST_CASE("sock_addr_recv_accept_advanced_validation", "[netebpfext][recv_accept]")
{
    // This test validates advanced recv_accept scenarios including context validation,
    // compartment ID handling, and validation flag behavior to ensure complete parity
    // with connect hook testing coverage.
    ebpf_extension_data_t npi_specific_characteristics = {
        .header = EBPF_ATTACH_CLIENT_DATA_HEADER_VERSION,
    };
    test_sock_addr_client_context_header_t client_context_header = {0};
    ...
    // Test recv_accept with validation enabled vs disabled
    client_context->sock_addr_action = SOCK_ADDR_TEST_ACTION_PERMIT_HARD;
    ...
    // Test recv_accept with various reauthorization scenarios
    client_context->validate_sock_addr_entries = false;
    client_context->sock_addr_action = SOCK_ADDR_TEST_ACTION_PERMIT_SOFT;
    ...
    // Test recv_accept with different source/destination port combinations
    client_context->sock_addr_action = SOCK_ADDR_TEST_ACTION_ROUND_ROBIN;
    parameters.reauthorization_flag = 0;

    // Test port ranges to ensure recv_accept handles port-based decisions correctly
    std::vector<std::pair<uint16_t, uint16_t>> port_combinations = {{80, 8080}, {443, 8443}, {1234, 5678}, {0, 65535}};

    for (const auto& [src_port, dst_port] : port_combinations) {
        parameters.source_port = htons(src_port);
        parameters.destination_port = htons(dst_port);

        FWP_ACTION_TYPE expected_action = _get_fwp_sock_addr_action(dst_port);

        result = helper.test_cgroup_inet4_recv_accept(&parameters);
        REQUIRE(result == expected_action);

        result = helper.test_cgroup_inet6_recv_accept(&parameters);
        REQUIRE(result == expected_action);
    }

    // Test recv_accept with edge case scenarios
    client_context->sock_addr_action = SOCK_ADDR_TEST_ACTION_PERMIT_HARD;

    // Test with maximum port values
    parameters.source_port = htons(65535);
    parameters.destination_port = htons(65535);
    result = helper.test_cgroup_inet4_recv_accept(&parameters);
    REQUIRE(result == FWP_ACTION_PERMIT);

    // Test with minimum port values
    parameters.source_port = htons(0);
    parameters.destination_port = htons(0);
    result = helper.test_cgroup_inet4_recv_accept(&parameters);
    REQUIRE(result == FWP_ACTION_PERMIT);

    // Verify recv_accept handles all action types correctly
    ...
    for (int action : all_actions) {
        client_context->sock_addr_action = action;

        // Test should not crash regardless of action type
        helper.test_cgroup_inet4_recv_accept(&parameters);
        helper.test_cgroup_inet6_recv_accept(&parameters);
    }
}
