...
typedef class _ebpf_map_test_state
{
  public:
    _ebpf_map_test_state(ebpf_map_type_t type)
    {
        ...
        ebpf_map_definition_t definition{sizeof(ebpf_map_definition_t), type, sizeof(uint32_t), sizeof(uint64_t), 1};

        REQUIRE(ebpf_map_create(&name, &definition, &map) == EBPF_SUCCESS);
    }
