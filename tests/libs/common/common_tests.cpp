...
ebpf_test_pinned_map_enum()
{
    ...
    for (int i = 0; i < pinned_map_count; i++) {
        ...
        // The canonical file path actually pinned should have a drive letter,
        // so should be 2 longer than the non-canonical path.
        REQUIRE(
            (matched =
                 (static_cast<uint16_t>(pin_path.size() + 2) ==
