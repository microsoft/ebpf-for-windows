...
void
_test_nested_maps2(bpf_map_type map_type)
{
    _test_helper_end_to_end test_helper;
    test_helper.initialize();
    ...
    key = 0;
    uint32_t value1 = 100;
    // Insert this value into inner_map1
    result = bpf_map_update_elem(inner_map_fd1, &key, &value1, 0);
    REQUIRE(result == ERROR_SUCCESS);

    uint32_t value2 = 1000;
    // Insert this value into inner_map2
    result = bpf_map_update_elem(inner_map_fd2, &key, &value2, 0);
    REQUIRE(result == ERROR_SUCCESS);
    ...
    // Get the inner_map1 ID from outer_map
    key = 1;
    uint32_t inner_map_id1 = 0;
    result = bpf_map_lookup_elem(outer_map_fd, &key, &inner_map_id1);
    REQUIRE(result == 0);
    REQUIRE(inner_map_id1 > 0);
    inner_map_fd1 = bpf_map_get_fd_by_id(inner_map_id1);
    REQUIRE(inner_map_fd1 > 0);
    // Query value from inner_map1
    key = 0;
    uint32_t value = 0;
    result = bpf_map_lookup_elem(inner_map_fd1, &key, &value);
    REQUIRE(result == ERROR_SUCCESS);
    REQUIRE(value == value1);
    Platform::_close(inner_map_fd1);

    // Get the inner_map2 ID from outer_map
    key = 2;
    uint32_t inner_map_id2 = 0;
    result = bpf_map_lookup_elem(outer_map_fd, &key, &inner_map_id2);
    REQUIRE(result == 0);
    REQUIRE(inner_map_id2 > 0);
    inner_map_fd2 = bpf_map_get_fd_by_id(inner_map_id2);
    REQUIRE(inner_map_fd2 > 0);
    // Query value from inner_map2
    key = 0;
    value = 0;
    result = bpf_map_lookup_elem(inner_map_fd2, &key, &value);
    REQUIRE(result == ERROR_SUCCESS);
...
TEST_CASE("array_map_of_maps2", "[libbpf]") { _test_nested_maps2(BPF_MAP_TYPE_ARRAY_OF_MAPS); }
TEST_CASE("hash_map_of_maps2", "[libbpf]") { _test_nested_maps2(BPF_MAP_TYPE_HASH_OF_MAPS); }
