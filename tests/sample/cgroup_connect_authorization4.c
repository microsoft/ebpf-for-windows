...
} connect_authorization4_count_map SEC(".maps");

static const uint16_t remote_port = SOCKET_TEST_PORT;

SEC("cgroup/connect_authorization4")
int
count_tcp_connect_authorization4(bpf_sock_addr_t* ctx)
{
    ...
    uint64_t value = 0;
    uint64_t* count = bpf_map_lookup_elem(&connect_authorization4_count_map, &key);
    if (!count) {
        value = 1;
        bpf_map_update_elem(&connect_authorization4_count_map, &key, &value, BPF_ANY);
    } else {
        value = *count + 1;
        bpf_map_update_elem(&connect_authorization4_count_map, &key, &value, BPF_EXIST);
    }
