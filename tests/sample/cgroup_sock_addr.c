...
struct
{
    ...
    __type(value, uint32_t);
    __uint(max_entries, 1);
} listen_connection_policy_map SEC(".maps");
...
authorize_listen_v4(bpf_sock_addr_t* ctx, void* connection_policy_map)
{
    ...
    // For listen, use local address/port (user_* fields contain the local listen address).
    tuple_key.local_ip.ipv4 = ctx->user_ip4;
    tuple_key.local_port = ctx->user_port;
    tuple_key.protocol = ctx->protocol;
...
authorize_listen_v6(bpf_sock_addr_t* ctx, void* connection_policy_map)
{
    connection_tuple_t tuple_key = {0};
    int* verdict;
    __builtin_memcpy(tuple_key.local_ip.ipv6, ctx->user_ip6, sizeof(ctx->user_ip6));
    tuple_key.local_port = ctx->user_port;
    tuple_key.protocol = ctx->protocol;
...
SEC("cgroup/listen4")
int
authorize_listen4(bpf_sock_addr_t* ctx)
{
    return authorize_listen_v4(ctx, &listen_connection_policy_map);
}

SEC("cgroup/listen6")
int
authorize_listen6(bpf_sock_addr_t* ctx)
{
    return authorize_listen_v6(ctx, &listen_connection_policy_map);
}
