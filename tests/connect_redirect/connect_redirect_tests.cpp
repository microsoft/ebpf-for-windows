...
#include <ntsecapi.h>
    ...
    uint64_t _get_current_thread_authentication_id()
    {
        TOKEN_GROUPS_AND_PRIVILEGES* privileges = nullptr;
        uint32_t size = 0;
        HANDLE thread_token_handle = GetCurrentThreadEffectiveToken();
        ...
        bool result =
            GetTokenInformation(thread_token_handle, TokenGroupsAndPrivileges, nullptr, 0, (unsigned long*)&size);
        REQUIRE(GetLastError() == ERROR_INSUFFICIENT_BUFFER);
    ...
    static HANDLE _log_on_user(std::string & user_name, std::string & password)
    {
        HANDLE token = 0;
        if (_globals.user_type != user_type_t::ADMINISTRATOR) {
            bool result = LogonUserA(
    ...
    inline static IPPROTO _get_protocol_from_string(std::string protocol)
    {
        if (protocol.compare("udp") == 0 || protocol.compare("UDP") == 0) {
            return IPPROTO_UDP;
        } else if (protocol.compare("tcp") == 0 || protocol.compare("TCP") == 0) {
    ...
    static user_type_t _get_user_type(std::string & user_type_string)
    {
        if (user_type_string == "" || user_type_string == "Administrator") {
            return user_type_t::ADMINISTRATOR;
        }
    ...
    static void _initialize_test_globals()
    {
        if (_globals_initialized) {
            return;
        }
        ...
        if (_remote_ip_v4 != "") {
            ...
            REQUIRE(family == AF_INET);
            get_address_from_string(
                _remote_ip_v4, _globals.addresses[socket_family_t::Dual].remote_address, true, &family);
            REQUIRE(family == AF_INET);
            v4_addresses++;
        }
        if (_local_ip_v4 != "") {
            get_address_from_string(
                _local_ip_v4, _globals.addresses[socket_family_t::IPv4].local_address, false, &family);
            REQUIRE(family == AF_INET);
            get_address_from_string(
                _local_ip_v4, _globals.addresses[socket_family_t::Dual].local_address, true, &family);
            REQUIRE(family == AF_INET);
            v4_addresses++;
        }
        if (_vip_v4 != "") {
        ...
        if (_local_ip_v6 != "") {
            get_address_from_string(
                _local_ip_v6, _globals.addresses[socket_family_t::IPv6].local_address, false, &family);
            REQUIRE(family == AF_INET6);
            v6_addresses++;
        }
        if (_vip_v6 != "") {
    ...
    static void _validate_audit_map_entry(_In_ const struct bpf_object* object, uint64_t authentication_id)
    {
        bpf_map* audit_map = bpf_object__find_map_by_name(object, "audit_map");
        REQUIRE(audit_map != nullptr);
    ...
    static void _load_and_attach_ebpf_programs(_Outptr_ struct bpf_object * *return_object)
    {
        ...
        if (_globals.attach_v4_program) {
            ...
            bpf_program* connect_program_v4 =
                bpf_object__find_program_by_name(unique_object.get(), "connect_redirect4");
        ...
        if (_globals.attach_v6_program) {
            ...
            bpf_program* connect_program_v6 =
                bpf_object__find_program_by_name(unique_object.get(), "connect_redirect6");
    ...
    static void _update_policy_map(
    ...
    void get_client_socket(bool dual_stack, _Inout_ client_socket_t** sender_socket)
    {
        impersonation_helper_t helper(_globals.user_type);
    ...
    void authorize_test_wrapper(
        _In_ const struct bpf_object* object, bool dual_stack, _In_ sockaddr_storage& destination)
    {
    ...
    void connect_redirect_test_wrapper(
        _In_ const struct bpf_object* object,
        _In_ sockaddr_storage& destination,
        _In_ sockaddr_storage& proxy,
        bool dual_stack)
