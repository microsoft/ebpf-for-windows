...
prepare_ubpf_vm(const std::vector<ebpf_inst> instructions)
{
    ...
    for (auto& [key, value] : helper_functions) {
        REQUIRE(ubpf_register(vm, key, "unnamed", reinterpret_cast<external_function_t>(value)) == 0);
    }
...
run_bpf_code_generator_test(const std::string& data_file)
{
    ...
    std::string include_args = std::string(" -I.." SEPARATOR ".." SEPARATOR "include") +
                               std::string(" -I.." SEPARATOR ".." SEPARATOR "undocked" SEPARATOR "tests" SEPARATOR
                                           "sample" SEPARATOR "ext" SEPARATOR "inc");
    std::string compile_command =
        cc + std::string(" ") + cxxflags + include_args + std::string(" /Zi /c /Fo") + generated_object +
        std::string(" /Fd") + generated_pdb + std::string(" ") + prefix + std::string(".c >") + log_file +
        std::string(" 2>&1 && ") + cc + std::string(" ") + cxxflags + include_args + std::string(" /Zi /c /Fo") +
        test_object + std::string(" /Fd") + test_pdb + std::string(" bpf_test.cpp >>") + log_file +
        std::string(" 2>&1 && ") + cc + std::string(" ") + cxxflags + std::string(" /Fe") + prefix + std::string(" ") +
        generated_object + std::string(" ") + test_object + std::string(" >>") + log_file + std::string(" 2>&1");
