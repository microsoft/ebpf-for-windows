...
static void
_test_map_of_maps_lookup_and_delete(bpf_map_type map_type)
{
    _test_helper_end_to_end test_helper;
    test_helper.initialize();

    REQUIRE((map_type == BPF_MAP_TYPE_ARRAY_OF_MAPS || map_type == BPF_MAP_TYPE_HASH_OF_MAPS));

    fd_t inner_map_fd = bpf_map_create(BPF_MAP_TYPE_ARRAY, "inner_map", sizeof(uint32_t), sizeof(uint32_t), 1, nullptr);
    REQUIRE(inner_map_fd > 0);

    bpf_map_create_opts opts = {.inner_map_fd = (uint32_t)inner_map_fd};
    fd_t outer_map_fd = bpf_map_create(map_type, "outer_map", sizeof(uint32_t), sizeof(fd_t), 1, &opts);
    REQUIRE(outer_map_fd > 0);

    uint32_t key = 0;
    REQUIRE(bpf_map_update_elem(outer_map_fd, &key, &inner_map_fd, 0) == 0);

    uint32_t inner_map_id = 0;
    // bpf_map_lookup_and_delete_elem should fail.
    REQUIRE(bpf_map_lookup_and_delete_elem(outer_map_fd, &key, &inner_map_id) != 0);
    REQUIRE(bpf_map_lookup_elem(outer_map_fd, &key, &inner_map_id) == 0);

    fd_t inner_map_check_fd = bpf_map_get_fd_by_id(inner_map_id);
    REQUIRE(inner_map_check_fd > 0);
    Platform::_close(inner_map_check_fd);
...
TEST_CASE("map_of_maps_lookup_and_delete", "[libbpf]")
{
    _test_map_of_maps_lookup_and_delete(BPF_MAP_TYPE_ARRAY_OF_MAPS);
    _test_map_of_maps_lookup_and_delete(BPF_MAP_TYPE_HASH_OF_MAPS);
}