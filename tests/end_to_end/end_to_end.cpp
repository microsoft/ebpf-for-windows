...
TEST_CASE("concurrent_pin_unpin_stress", "[end_to_end][stress]")
{
    _test_helper_end_to_end test_helper;
    test_helper.initialize();

    const char* map_pin_path = "/ebpf/global/stress_map";
    std::atomic<bool> stop{false};
    size_t ebpf_object_get_failures = 0;
    size_t ebpf_object_get_successes = 0;

    // Thread 1: Repeatedly try to get the pinned object and close it.
    std::thread thread1([&]() {
        while (!stop.load(std::memory_order_relaxed)) {
            fd_t map_fd = ebpf_fd_invalid;
            ebpf_result_t result = ebpf_object_get(map_pin_path, &map_fd);
            if (result == EBPF_SUCCESS && map_fd != ebpf_fd_invalid) {
                Platform::_close(map_fd);
                ebpf_object_get_successes++;
            } else {
                // std::cout << "[thread1] ebpf_object_get failed with " << result << std::endl;
                ebpf_object_get_failures++;
            }
        }
    });

    // Thread 2: Repeatedly create a map, pin it, close the fd, and unpin it.
    std::thread thread2([&]() {
        while (!stop.load(std::memory_order_relaxed)) {
            fd_t map_fd =
                bpf_map_create(BPF_MAP_TYPE_ARRAY, "stress_map", sizeof(uint32_t), sizeof(uint32_t), 1, nullptr);
            if (map_fd < 0) {
                std::cout << "[thread2] bpf_map_create failed with " << map_fd << std::endl;
                continue;
            }
            if (bpf_obj_pin(map_fd, map_pin_path) != 0) {
                std::cout << "[thread2] bpf_obj_pin failed with " << errno << std::endl;
                Platform::_close(map_fd);
                continue;
            }
            Platform::_close(map_fd);
            ebpf_object_unpin(map_pin_path);
        }
    });

    // Let the threads race for 60 seconds.
    ...
    std::cout << "ebpf_object_get successes: " << ebpf_object_get_successes << std::endl;
    std::cout << "ebpf_object_get failures: " << ebpf_object_get_failures << std::endl;

    // Best-effort cleanup in case a pinned object is still around.
    ebpf_object_unpin(map_pin_path);
}
