...
static void
_test_hash_map_of_maps_lookup_and_delete()
{
    _test_helper_end_to_end test_helper;
    test_helper.initialize();
    ...
    bpf_map_create_opts opts = {.inner_map_fd = (uint32_t)inner_map_fd};
    fd_t outer_map_fd =
        bpf_map_create(BPF_MAP_TYPE_HASH_OF_MAPS, "outer_map", sizeof(uint32_t), sizeof(fd_t), 1, &opts);
    REQUIRE(outer_map_fd > 0);
...
TEST_CASE("hash_map_of_maps_lookup_and_delete", "[libbpf]") { _test_hash_map_of_maps_lookup_and_delete(); }