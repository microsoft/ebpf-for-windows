...
static ebpf_core_code_entry_t
    *_ebpf_core_hook_table[EBPF_PROGRAM_TYPE_BIND + 1] = {0};

// Assume enabled until we can query it
static ebpf_code_integrity_state_t _ebpf_core_code_integrity_state =
    EBPF_CODE_INTEGRITY_HYPER_VISOR_KERNEL_MODE;

static void *_ebpf_core_map_lookup_element(ebpf_core_map_t *map,
                                           const uint8_t *key);
static void _ebpf_core_map_update_element(ebpf_core_map_t *map,
                                          const uint8_t *key,
                                          const uint8_t *data);
static void _ebpf_core_map_delete_element(ebpf_core_map_t *map,
                                          const uint8_t *key);

static uint64_t ebpf_core_interpreter_helper_resolver(void *context,
                                                      uint32_t helper_id);

static const void *_ebpf_program_helpers[] = {
    NULL, (void *)&_ebpf_core_map_lookup_element,
...
_ebpf_core_set_hook_entry(ebpf_core_code_entry_t *code,
                          ebpf_program_type_t program_type) {
  ebpf_lock_state_t state;
  if (program_type > EBPF_PROGRAM_TYPE_BIND ||
...
ebpf_error_code_t ebpf_core_protocol_load_code(
  ...
  size_t code_size = request->header.length -
                     RTL_OFFSET_OF(ebpf_operation_load_code_request_t, code);
  size_t allocation_size = 0;
  ebpf_core_code_entry_t *code = NULL;
  ebpf_memory_type_t memory_type;
  retval = ebpf_safe_size_t_add(code_size, sizeof(ebpf_core_code_entry_t),
                                &allocation_size);
  ...
  if (request->code_type == EBPF_CODE_NATIVE) {
    if (_ebpf_core_code_integrity_state ==
        EBPF_CODE_INTEGRITY_HYPER_VISOR_KERNEL_MODE) {
  ...
  if (request->code_type == EBPF_CODE_NATIVE) {
    ...
    memcpy(code->code_or_vm.code, request->code,
    ...
    ubpf_register_helper_resolver(code->code_or_vm.vm, code,
                                  ebpf_core_interpreter_helper_resolver);
    if (ubpf_load(code->code_or_vm.vm, &request->code[0], (uint32_t)code_size,
                  &error_message) != 0) {
  ...
  retval = reply->handle != UINT64_MAX ? EBPF_ERROR_SUCCESS
...
ebpf_error_code_t ebpf_core_invoke_hook(_In_ ebpf_program_type_t hook_point,
  ...
  if (code) {
    if (code->code_type == EBPF_CODE_NATIVE) {
      ...
      *result = (uint32_t)(
          ubpf_exec(code->code_or_vm.vm, context, 1024, &error_message));
      if (error_message) {
        ebpf_free(error_message);
      }
      return EBPF_ERROR_SUCCESS;
...
ebpf_error_code_t ebpf_core_protocol_create_map(
  ...
  map =
      ebpf_map_function_tables[type].create_map(&request->ebpf_map_definition);
  if (map == NULL) {
    retval = EBPF_ERROR_OUT_OF_RESOURCES;
    goto Done;
  }
...
ebpf_error_code_t ebpf_core_protocol_map_lookup_element(
  ...
  if (request->header.length <
      (RTL_OFFSET_OF(ebpf_operation_map_lookup_element_request_t, key) +
  ...
  if (reply->header.length <
      (RTL_OFFSET_OF(ebpf_operation_map_lookup_element_reply_t, value) +
...
ebpf_error_code_t ebpf_core_protocol_map_update_element(
  ...
  if (request->header.length <
      (RTL_OFFSET_OF(epf_operation_map_update_element_request_t, data) +
       map->ebpf_map_definition.key_size +
       map->ebpf_map_definition.value_size)) {
    retval = EBPF_ERROR_INVALID_PARAMETER;
    goto Done;
...
ebpf_error_code_t ebpf_core_protocol_map_delete_element(
  ...
  if (request->header.length <
      (RTL_OFFSET_OF(ebpf_operation_map_delete_element_request_t, key) +
       map->ebpf_map_definition.key_size)) {
    retval = EBPF_ERROR_INVALID_PARAMETER;
    goto Done;
  }
...
ebpf_error_code_t ebpf_core_protocol_map_get_next_key(
  ...
  if (request->header.length ==
    ...
    previous_key = NULL;
  } else if (request->header.length <
             (RTL_OFFSET_OF(ebpf_operation_map_next_key_request_t,
                            previous_key) +
              map->ebpf_map_definition.key_size)) {
    retval = EBPF_ERROR_INVALID_PARAMETER;
    goto Done;
  ...
  if (reply->header.length <
      (RTL_OFFSET_OF(ebpf_operation_map_next_key_reply_t, next_key) +
       map->ebpf_map_definition.key_size)) {
    retval = EBPF_ERROR_INVALID_PARAMETER;
    goto Done;
  }
...
ebpf_error_code_t ebpf_core_protocol_enumerate_maps(
  ...
  ebpf_lock_state_t state;
  ebpf_lock_lock(&_ebpf_core_map_entry_table_lock, &state);
  for (; current_handle < RTL_COUNT_OF(_ebpf_core_map_entry_table);
       current_handle++) {
...
void _ebpf_core_map_update_element(ebpf_core_map_t *map, const uint8_t *key,
                                   const uint8_t *value) {
  size_t type = map->ebpf_map_definition.type;
