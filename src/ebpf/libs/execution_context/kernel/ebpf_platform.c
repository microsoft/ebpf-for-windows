...
ebpf_hash_map_compare(struct _RTL_AVL_TABLE* table, void* first_struct, void* second_struct)
{
...
ebpf_hash_table_create(ebpf_hash_table_t** hash_table, size_t key_size, size_t value_size)
{
    ...
    // allocate
    table = ebpf_allocate(sizeof(RTL_AVL_TABLE), EBPF_MEMORY_NO_EXECUTE);
    if (table == NULL) {
    ...
    RtlInitializeGenericTableAvl(
        table, ebpf_hash_map_compare, ebpf_hash_map_allocate, ebpf_hash_map_free, (void*)sizes);
