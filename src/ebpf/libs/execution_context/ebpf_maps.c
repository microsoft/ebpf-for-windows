...
ebpf_create_array_map(_In_ const ebpf_map_definition_t *map_definition) {
  ...
  retval = ebpf_safe_size_t_multiply(
      map_definition->max_entries, map_definition->value_size, &map_data_size);
  if (retval != EBPF_ERROR_SUCCESS) {
    goto Done;
  }

  retval =
      ebpf_safe_size_t_multiply(map_data_size, map_entry_size, &map_entry_size);
  if (retval != EBPF_ERROR_SUCCESS) {
    goto Done;
  }
...
static void ebpf_delete_array_map(_In_ ebpf_core_map_t *map) { ebpf_free(map); }

static uint8_t *ebpf_lookup_array_map_entry(_In_ ebpf_core_map_t *map,
...
static ebpf_error_code_t ebpf_update_array_map_entry(_In_ ebpf_core_map_t *map,
                                                     _In_ const uint8_t *key,
...
static ebpf_error_code_t ebpf_delete_array_map_entry(_In_ ebpf_core_map_t *map,
                                                     _In_ const uint8_t *key) {
  uint32_t key_value;
  if (!map || !key)
    return EBPF_ERROR_INVALID_PARAMETER;
...
ebpf_create_hash_map(_In_ const ebpf_map_definition_t *map_definition) {
  ...
  retval = ebpf_hash_table_create((ebpf_hash_table_t **)&map->data,
...
static void ebpf_delete_hash_map(_In_ ebpf_core_map_t *map) {
  ebpf_lock_destroy(&map->lock);
  ebpf_hash_table_destroy((ebpf_hash_table_t *)map->data);
  ebpf_free(map);
}

static uint8_t *ebpf_lookup_hash_map_entry(_In_ ebpf_core_map_t *map,
                                           _In_ const uint8_t *key) {
  ...
  if (ebpf_hash_table_lookup((ebpf_hash_table_t *)map->data, key, &value) !=
      EBPF_ERROR_SUCCESS) {
    value = NULL;
  }
...
static ebpf_error_code_t ebpf_update_hash_map_entry(_In_ ebpf_core_map_t *map,
                                                    _In_ const uint8_t *key,
                                                    _In_ const uint8_t *data) {
...
static ebpf_error_code_t ebpf_delete_hash_map_entry(_In_ ebpf_core_map_t *map,
                                                    _In_ const uint8_t *key) {
  ebpf_error_code_t result;
  ebpf_lock_state_t lock_state;
  if (!map || !key)
...
ebpf_next_hash_map_key(_In_ ebpf_core_map_t *map,
  ...
  ebpf_lock_lock(&map->lock, &lock_state);
  result = ebpf_hash_table_next_key((ebpf_hash_table_t *)map->data,
...
ebpf_map_function_table_t ebpf_map_function_tables[] = {
    {// EBPF_MAP_TYPE_UNSPECIFIED
     NULL},
    {// EBPF_MAP_TYPE_HASH
     ebpf_create_hash_map, ebpf_delete_hash_map, ebpf_lookup_hash_map_entry,
     ebpf_update_hash_map_entry, ebpf_delete_hash_map_entry,
     ebpf_next_hash_map_key},
    {// EBPF_MAP_TYPE_ARRAY
     ebpf_create_array_map, ebpf_delete_array_map, ebpf_lookup_array_map_entry,
     ebpf_update_array_map_entry, ebpf_delete_array_map_entry, NULL},
};