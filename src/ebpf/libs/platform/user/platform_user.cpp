...
void ebpf_free(void *memory) {
  if (_executable_segments.find(reinterpret_cast<uint64_t>(memory)) !=
      _executable_segments.end()) {
...
void ebpf_hash_table_destroy(ebpf_hash_table_t *hash_table) {
  hash_table_t *local_hash_table =
      reinterpret_cast<decltype(local_hash_table)>(hash_table);
...
ebpf_error_code_t ebpf_hash_table_next_key(ebpf_hash_table_t *hash_table,
  ...
  if (previous_key) {
    std::vector<uint8_t> local_key(previous_key,
                                   previous_key + local_hash_table->key_size);
