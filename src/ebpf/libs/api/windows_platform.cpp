...
static EbpfProgramType get_program_type_windows(const std::string &section,
                                                const std::string &) {
...
EbpfMapType get_map_type_windows(uint32_t platform_specific_type) {
  ...
  if ((index == 0) ||
      (index >= sizeof(windows_map_types) / sizeof(windows_map_types[0]))) {
...
int create_map_function(uint32_t type, uint32_t key_size, uint32_t value_size,
                        uint32_t max_entries, ebpf_verifier_options_t options);

static int create_map_windows(uint32_t map_type, uint32_t key_size,
  ...
  return create_map_function(map_type, key_size, value_size, max_entries,
                             options);
}

void parse_maps_section_windows(std::vector<EbpfMapDescriptor> &map_descriptors,
  ...
  auto mapdefs = std::vector<ebpf_maps_section_record_windows>(
      (ebpf_maps_section_record_windows *)data,
      (ebpf_maps_section_record_windows *)(data + size));
  for (auto s : mapdefs) {
    map_descriptors.emplace_back(EbpfMapDescriptor{
        .original_fd = create_map_windows(s.type, s.key_size, s.value_size,
                                          s.max_entries, options),
        .type = s.type,
        .key_size = s.key_size,
...
const ebpf_platform_t g_ebpf_platform_windows = {
    get_program_type_windows,   get_helper_prototype_windows,
    is_helper_usable_windows,   sizeof(ebpf_maps_section_record_windows),
    parse_maps_section_windows, get_map_descriptor_windows,
};
