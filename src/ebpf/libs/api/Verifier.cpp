...
int get_file_size(const char *filename, size_t *byte_code_size) {
  int result = 0;
  *byte_code_size = NULL;
  struct stat st = {0};
...
static const char *allocate_error_string(const std::string &str) {
  char *retval;
...
static int analyze(raw_program &raw_prog, const char **error_message) {
  ...
  if (!std::holds_alternative<InstructionSeq>(prog_or_error)) {
    *error_message =
        allocate_error_string(std::get<std::string>(prog_or_error));
    return 1; // Error;
  }
...
int verify(const char *filename, const char *sectionname, uint8_t *byte_code,
           size_t *byte_code_size, const char **error_message) {
...
int verify_byte_code(const char *path, const char *section_name,
                     const uint8_t *byte_code, size_t byte_code_size,
                     const char **error_message) {
  const ebpf_platform_t *platform = &g_ebpf_platform_windows;
  std::vector<ebpf_inst> instructions{(ebpf_inst *)byte_code,
                                      (ebpf_inst *)byte_code +
                                          byte_code_size / sizeof(ebpf_inst)};
...
uint32_t ebpf_api_elf_enumerate_sections(
    const char *file, const char *section, bool verbose,
    const struct _tlv_type_length_value **data, const char **error_message) {
  ebpf_verifier_options_t verifier_options{false, false, false, false, true};
  const ebpf_platform_t *platform = &g_ebpf_platform_windows;
  std::ostringstream str;
  try {
    auto raw_programs =
        read_elf(file, section ? std::string(section) : std::string(),
                 &verifier_options, platform);
    tlv_sequence sequence;
    for (const auto &raw_program : raw_programs) {
      tlv_sequence stats_sequence;
      if (verbose) {
        std::variant<InstructionSeq, std::string> programOrError =
            unmarshal(raw_program, platform);
        if (std::holds_alternative<std::string>(programOrError)) {
          std::cout << "parse failure: "
                    << std::get<std::string>(programOrError) << "\n";
          return 1;
        }
        ...
        for (const auto &kvp : stats) {
          stats_sequence.emplace_back(tlv_pack<tlv_sequence>(
              {tlv_pack(kvp.first.c_str()), tlv_pack(kvp.second)}));
        }
      }
    ...
    auto local_data =
        reinterpret_cast<tlv_type_length_value_t *>(malloc(retval.size()));
...
uint32_t ebpf_api_elf_disassemble_section(const char *file, const char *section,
...
uint32_t ebpf_api_elf_verify_section(const char *file, const char *section,
                                     const char **report,
                                     const char **error_message) {
  std::ostringstream error;
  std::ostringstream output;
  try {
    ...
    bool res = ebpf_verify_program(output, program, raw_program.info,
                                   &verifier_options);
    if (!res) {
...
void ebpf_api_elf_free(const tlv_type_length_value_t *data) {
