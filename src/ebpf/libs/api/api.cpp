...
static uint32_t invoke_ioctl(ebpf_handle_t handle, request_t &request,
  ...
  auto result = Platform::DeviceIoControl(
      handle, IOCTL_EBPFCTL_METHOD_BUFFERED, request_ptr, request_size,
      reply_ptr, reply_size, &actual_reply_size, nullptr);
...
uint32_t ebpf_api_initiate() {
  ...
  device_handle =
      Platform::CreateFile(ebpfDeviceName, GENERIC_READ | GENERIC_WRITE, 0,
...
int create_map_function(uint32_t type, uint32_t key_size, uint32_t value_size,
                        uint32_t max_entries, ebpf_verifier_options_t) {
  _ebpf_operation_create_map_request request{
      ...
      {sizeof(struct _ebpf_map_definition), type, key_size, value_size,
       max_entries}};
  ...
  if (retval != ERROR_SUCCESS) {
    throw std::runtime_error(std::string("Error ") + std::to_string(retval) +
                             " trying to create map");
  }

  if (reply.header.id != ebpf_operation_id_t::EBPF_OPERATION_CREATE_MAP) {
    throw std::runtime_error(std::string(
        "reply.header.id != ebpf_operation_id_t::EBPF_OPERATION_CREATE_MAP"));
  }
  ...
  _map_file_descriptors.push_back(
      {reply.handle, {fd, type, key_size, value_size, 0}});
...
static uint64_t map_resolver(void *context, uint64_t fd) {
  _ebpf_operation_resolve_map_request request{
      sizeof(request), ebpf_operation_id_t::EBPF_OPERATION_RESOLVE_MAP,
...
static uint64_t helper_resolver(void *context, uint32_t helper) {
  _ebpf_operation_resolve_helper_request request{
      sizeof(request), ebpf_operation_id_t::EBPF_OPERATION_RESOLVE_HELPER,
      helper};
...
static uint32_t resolve_maps_in_byte_code(std::vector<uint8_t> &byte_code) {
  ...
  ebpf_inst *instruction_end =
      reinterpret_cast<ebpf_inst *>(byte_code.data() + byte_code.size());
  for (size_t index = 0; index < byte_code.size() / sizeof(ebpf_inst);
       index++) {
    ...
    uint64_t imm = static_cast<uint64_t>(first_instruction.imm) |
                   (static_cast<uint64_t>(second_instruction.imm) << 32);
...
uint32_t ebpf_api_load_program(const char *file_name, const char *section_name,
  ...
  try {
    ...
    // Verify code.
    if (verify(file_name, section_name, byte_code.data(), &byte_code_size,
               error_message) != 0) {
      return ERROR_INVALID_PARAMETER;
    }
    ...
    char *error =
        reinterpret_cast<char *>(calloc(message_length + 1, sizeof(char)));
    if (error) {
  ...
  *count_of_map_handles = 0;
  ...
  if (execution_type == EBPF_EXECUTION_JIT) {
    ...
    if (ubpf_load(vm, byte_code.data(), static_cast<uint32_t>(byte_code.size()),
                  const_cast<char **>(error_message)) < 0) {
      return ERROR_INVALID_PARAMETER;
    }

    if (ubpf_translate(vm, machine_code.data(), &machine_code_size,
                       const_cast<char **>(error_message))) {
      return ERROR_INVALID_PARAMETER;
    }
    machine_code.resize(machine_code_size);

    request_buffer.resize(machine_code.size() +
                          offsetof(ebpf_operation_load_code_request_t, code));
    auto request = reinterpret_cast<ebpf_operation_load_code_request_t *>(
        request_buffer.data());
    ...
    request_buffer.resize(byte_code.size() +
                          offsetof(ebpf_operation_load_code_request_t, code));
    auto request = reinterpret_cast<ebpf_operation_load_code_request_t *>(
        request_buffer.data());
    request->header.id = ebpf_operation_id_t::EBPF_OPERATION_LOAD_CODE;
    ...
    std::copy(byte_code.begin(), byte_code.end(),
              request_buffer.begin() +
                  offsetof(ebpf_operation_load_code_request_t, code));
  }
...
void ebpf_api_unload_program(ebpf_handle_t handle) {
  _ebpf_operation_unload_code_request request{
      sizeof(request), ebpf_operation_id_t::EBPF_OPERATION_UNLOAD_CODE,
...
uint32_t ebpf_api_attach_program(ebpf_handle_t handle,
                                 ebpf_program_type_t hook_point) {
  _ebpf_operation_attach_detach_request request{
      sizeof(request), ebpf_operation_id_t::EBPF_OPERATION_ATTACH_CODE,
      reinterpret_cast<uint64_t>(handle), hook_point};

  return invoke_ioctl(device_handle, request);
}

uint32_t ebpf_api_detach_program(ebpf_handle_t handle,
                                 ebpf_program_type_t hook_point) {
  _ebpf_operation_attach_detach_request request{
      sizeof(request), ebpf_operation_id_t::EBPF_OPERATION_DETACH_CODE,
      reinterpret_cast<uint64_t>(handle), hook_point};

  return invoke_ioctl(device_handle, request);
}

uint32_t ebpf_api_map_lookup_element(ebpf_handle_t handle, uint32_t key_size,
                                     const uint8_t *key, uint32_t value_size,
                                     uint8_t *value) {
  std::vector<uint8_t> request_buffer(
      sizeof(_ebpf_operation_map_lookup_element_request) + key_size - 1);
  std::vector<uint8_t> reply_buffer(
      sizeof(_ebpf_operation_map_lookup_element_reply) + value_size - 1);
  auto request = reinterpret_cast<_ebpf_operation_map_lookup_element_request *>(
      request_buffer.data());
  auto reply = reinterpret_cast<_ebpf_operation_map_lookup_element_reply *>(
      reply_buffer.data());
  ...
  if (reply->header.id !=
      ebpf_operation_id_t::EBPF_OPERATION_MAP_LOOKUP_ELEMENT) {
    return ERROR_INVALID_PARAMETER;
  }
...
uint32_t ebpf_api_map_update_element(ebpf_handle_t handle, uint32_t key_size,
                                     const uint8_t *key, uint32_t value_size,
                                     const uint8_t *value) {
  std::vector<uint8_t> request_buffer(
      sizeof(_ebpf_operation_map_update_element_request) - 1 + key_size +
      value_size);
  auto request = reinterpret_cast<_ebpf_operation_map_update_element_request *>(
      request_buffer.data());
...
uint32_t ebpf_api_map_delete_element(ebpf_handle_t handle, uint32_t key_size,
                                     const uint8_t *key) {
  std::vector<uint8_t> request_buffer(
      sizeof(_ebpf_operation_map_delete_element_request) - 1 + key_size);
  auto request = reinterpret_cast<_ebpf_operation_map_delete_element_request *>(
      request_buffer.data());
...
uint32_t ebpf_api_map_next_key(ebpf_handle_t handle, uint32_t key_size,
                               const uint8_t *previous_key, uint8_t *next_key) {
  std::vector<uint8_t> request_buffer(
      offsetof(ebpf_operation_map_next_key_request_t, previous_key) + key_size);
  std::vector<uint8_t> reply_buffer(
      offsetof(ebpf_operation_map_next_key_reply_t, next_key) + key_size);
  auto request = reinterpret_cast<ebpf_operation_map_next_key_request_t *>(
      request_buffer.data());
  auto reply = reinterpret_cast<ebpf_operation_map_next_key_reply_t *>(
      reply_buffer.data());
...
uint32_t ebpf_api_map_enumerate(ebpf_handle_t previous_handle,
                                ebpf_handle_t *next_handle) {
  _ebpf_operation_enumerate_maps_request request{
      sizeof(request), ebpf_operation_id_t::EBPF_OPERATION_ENUMERATE_MAPS,
...
uint32_t ebpf_api_map_query_definition(ebpf_handle_t handle, uint32_t *size,
                                       uint32_t *type, uint32_t *key_size,
                                       uint32_t *value_size,
                                       uint32_t *max_entries) {
  _ebpf_operation_query_map_definition_request request{
      sizeof(request), ebpf_operation_id_t::EBPF_OPERATION_QUERY_MAP_DEFINITION,
      reinterpret_cast<uint64_t>(handle)};
...
void ebpf_api_delete_map(ebpf_handle_t handle) { CloseHandle(handle); }
