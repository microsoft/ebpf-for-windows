...
extern "C" {
     ...
     * @param[in] execution_type How this program should be run in the exeuction context.
     ...
     */
    uint32_t ebpf_api_map_lookup_element(
        ebpf_handle_t handle, 
        uint32_t key_size, 
        const uint8_t* key, 
        uint32_t value_size, 
     ...
     */
    uint32_t ebpf_api_map_update_element(ebpf_handle_t handle, 
        uint32_t key_size, 
        const uint8_t* key, 
        uint32_t value_size, 
        const uint8_t* value);
     ...
     */
    uint32_t ebpf_api_map_query_definition(ebpf_handle_t handle, 
        uint32_t* size, 
        uint32_t* type, 
        uint32_t* key_size, 
        uint32_t* value_size, 
     ...
     *   }\n 
     */
    uint32_t ebpf_api_elf_enumerate_sections(const char* file, 
        const char* section, 
        bool verbose, 
     ...
     *  the error.
     */
    uint32_t ebpf_api_elf_verify_section(const char* file, 
        const char* section, 
        const char** report, 
        const char** error_message);
