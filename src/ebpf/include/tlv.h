...
typedef enum class _tlv_type { BLOB, UINT, INT, STRING, SEQUENCE } tlv_type_t;
...
inline inner_type tlv_value(const tlv_type_length_value_t *tlv) {
  if constexpr (std::is_same<inner_type, std::string>::value) {
    std::string value;
    value.insert(0, reinterpret_cast<const char *>(tlv->value),
                 tlv->length - offsetof(tlv_type_length_value_t, value));
    return value;
  } else {
