...
inline std::vector<uint8_t> tlv_pack(const data_type_t& data)
{
...
inline const tlv_type_length_value_t* tlv_next(const tlv_type_length_value_t* current_tlv)
{
    return reinterpret_cast<const tlv_type_length_value_t*>(reinterpret_cast<const uint8_t*>(current_tlv) + current_tlv->length);
}

inline const tlv_type_length_value_t* tlv_child(const tlv_type_length_value_t* current_tlv)
{
    return reinterpret_cast<const tlv_type_length_value_t*>(current_tlv->value);
}

template<typename inner_type>
inline inner_type tlv_value(const tlv_type_length_value_t* tlv)
{
    if constexpr (std::is_same<inner_type, std::string>::value)
    {
        std::string value;
        value.insert(0, reinterpret_cast<const char*>(tlv->value), tlv->length - offsetof(tlv_type_length_value_t, value));
        return value;
    }
    else
    {
