...
// Pre-Declarations
//
static VOID EbpfCoreEvtIoDeviceControl(_In_ WDFQUEUE queue,
...
_Function_class_(EVT_WDF_DRIVER_UNLOAD) _IRQL_requires_same_
    _IRQL_requires_max_(PASSIVE_LEVEL) void EvtDriverUnload(
...
//
static NTSTATUS
EbpfCoreInitDriverObjects(_Inout_ DRIVER_OBJECT *driver_object,
                          _In_ const UNICODE_STRING *registry_path,
                          _Out_ WDFDRIVER *driver, _Out_ WDFDEVICE *device) {
  NTSTATUS status;
  ...
  status =
      WdfDriverCreate(driver_object, registry_path, WDF_NO_OBJECT_ATTRIBUTES,
                      &driver_configuration, driver);
  ...
  WdfDeviceInitSetCharacteristics(device_initialize, FILE_DEVICE_SECURE_OPEN,
                                  FALSE);
  ...
  status =
      WdfDeviceCreate(&device_initialize, WDF_NO_OBJECT_ATTRIBUTES, device);
  ...
  status = WdfIoQueueCreate(*device, &io_queue_configuration,
    ...
    if (input_buffer_length != 0) {
      ...
      if (!NT_SUCCESS(status)) {
        KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL,
                   "EbpfCore: Input buffer failure %d\n", status));
        goto Done;
      }
      ...
      if (input_buffer != NULL) {
        ...
        if (user_request->id >=
            sizeof(EbpfProtocolHandlers) / sizeof(EbpfProtocolHandlers[0])) {
          status = STATUS_INVALID_PARAMETER;
          goto Done;
        }

        if (user_request->length <
            EbpfProtocolHandlers[user_request->id].minimum_request_size) {
          status = STATUS_INVALID_PARAMETER;
          goto Done;
        }
        ...
        if (EbpfProtocolHandlers[user_request->id].minimum_reply_size > 0) {
          ...
          status = WdfRequestRetrieveOutputBuffer(request, output_buffer_length,
                                                  &output_buffer,
                                                  &actual_output_length);
          if (!NT_SUCCESS(status)) {
            KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL,
                       "EbpfCore: Output buffer failure %d\n", status));
            goto Done;
          }
          ...
          if (actual_output_length <
              EbpfProtocolHandlers[user_request->id].minimum_reply_size) {
        ...
        status = ebpf_error_code_to_ntstatus(
            EbpfProtocolHandlers[user_request->id].protocol_handler(
...
NTSTATUS
DriverEntry(_In_ DRIVER_OBJECT *driver_object,
  ...
  KdPrintEx(
      (DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL, "EbpfCore: DriverEntry\n"));

  status =
      EbpfCoreInitDriverObjects(driver_object, registry_path, &driver, &device);
