...
static DEVICE_OBJECT* _ebpf_wfp_ext_driver_device_object;
static BOOLEAN _ebpf_wfp_ext_driver_unloading_flag = FALSE;
...
static _Function_class_(EVT_WDF_DRIVER_UNLOAD) _IRQL_requires_same_
    _IRQL_requires_max_(PASSIVE_LEVEL) void _ebpf_wfp_ex_driver_unload(_In_ WDFDRIVER driver_object)
{
    UNREFERENCED_PARAMETER(driver_object);

    _ebpf_wfp_ext_driver_unloading_flag = TRUE;
...
//
static NTSTATUS
_ebpf_wfp_ext_driver_initialize_objects(
    ...
    _Out_ WDFDEVICE* device)
{
    ...
    driver_configuration.EvtDriverUnload = _ebpf_wfp_ex_driver_unload;
    ...
    RtlInitUnicodeString(&ebpf_device_name, EBPF_WFP_EXT_DEVICE_NAME);
...
DriverEntry(_In_ DRIVER_OBJECT* driver_object, _In_ UNICODE_STRING* registry_path)
{
    ...
    KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL, "EbpfWfpExt: DriverEntry\n"));

    status = _ebpf_wfp_ext_driver_initialize_objects(driver_object, registry_path, &driver, &device);

    if (!NT_SUCCESS(status)) {
        goto Exit;
    }

    _ebpf_wfp_ext_driver_device_object = WdfDeviceWdmGetDeviceObject(device);

    ebpf_hook_register_callouts(_ebpf_wfp_ext_driver_device_object);
