...
DWORD handle_ebpf_show_disassembly(LPCWSTR machine, LPWSTR *argv,
  ...
  if (ebpf_api_elf_disassemble_section(filename.c_str(), section.c_str(),
                                       &disassembly, &error_message) != 0) {
...
DWORD handle_ebpf_show_sections(LPCWSTR machine, LPWSTR *argv,
                                DWORD current_index, DWORD argc, DWORD flags,
                                LPCVOID data, BOOL *done) {
  UNREFERENCED_PARAMETER(machine);
  UNREFERENCED_PARAMETER(flags);
  ...
  const char *error_message = nullptr;
  if (ebpf_api_elf_enumerate_sections(filename.c_str(), section.c_str(),
                                      level == VL_VERBOSE, &section_data,
                                      &error_message) != 0) {
    std::cerr << error_message << std::endl;
    ebpf_api_free_error_message(error_message);
  ...
  for (auto current_section = tlv_child(section_data);
    ...
    if (level == VL_NORMAL) {
      std::cout << std::setw(20) << std::right
                << tlv_value<std::string>(section_name) << "  " << std::setw(6)
                << tlv_value<uint64_t>(type) << "  " << std::setw(6)
                << tlv_value<size_t>(map_count) << "  " << std::setw(6)
                << (program_bytes->length -
                    offsetof(tlv_type_length_value_t, value)) /
                       8
                << "\n";
    } else {
      std::cout << "\n";
      ...
      std::cout << "Size         : "
                << program_bytes->length -
                       offsetof(tlv_type_length_value_t, value) / 8
                << " instructions\n";
      for (auto stat = tlv_child(stats_secton);
           stat != tlv_next(current_section); stat = tlv_next(stat)) {
        auto key = tlv_child(stat);
        auto value = tlv_next(key);
        std::cout << std::setw(13) << std::left << tlv_value<std::string>(key)
                  << ": " << tlv_value<int>(value) << "\n";
      }
    }
  }
...
DWORD handle_ebpf_show_verification(LPCWSTR machine, LPWSTR *argv,
  ...
  status = ebpf_api_elf_verify_section(filename.c_str(), section.c_str(),
